...
Extension → webview: `{type: 'update', html, toc, hasMermaid, fileName, wordCount, config}`, `{type: 'config', config}` (live setting broadcast), `{type: 'scrollTo', percentage}`, `{type: 'toggleTOC'}`, `{type: 'toggleSettings'}`, `{type: 'find'}` (focuses/opens the find bar — triggered by the `mdReader.find` command, Ctrl/Cmd+F), `{type: 'print'}` (closes any open overlay, then `window.print()` — triggered by `mdReader.exportPdf`; the reader's own `@media print` rules in reader.css hide chrome/force light colors for the resulting PDF/printout).
